...
class Comment: NSManagedObject {
    ...
    class func getCommentInfo(postId: Int) -> [Comment] {
        ...
        let items = CoreDataOperation.objectsForEntity(EntityName.Comment, context: context(), filter: predicate, sort: nil) as! [Comment]
        return items
    }
    // MARK:- INSERT
    class func insert(comments: [AnyObject]) {
        let entity = NSEntityDescription.entityForName(EntityName.Comment, inManagedObjectContext: context())
        for comment in comments {
            let item = Comment(entity: entity!, insertIntoManagedObjectContext: context())
