...
class Post: NSManagedObject {
    ...
    class func getPostInfo() -> [Post] {
        let items = CoreDataOperation.objectsForEntity(EntityName.Post, context: context(), filter: nil, sort: nil) as! [Post]
        return items
    }
    // MARK:- INSERT
    class func insert(posts: [AnyObject]){
        let entity = NSEntityDescription.entityForName(EntityName.Post, inManagedObjectContext: context())
        for post in posts {
            let item = Post(entity: entity!, insertIntoManagedObjectContext: context())
